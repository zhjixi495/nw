...
int IniFile::getIntValue(const string &section, const string &key, int &ret,int minValue,int maxValue,int defaultValue,const string &usrcomment)
{
    string value, comment;
    int curValue = -1;
    bool changeFlag = 0;
    ...
    {
        char tempstring[64];
        sprintf(tempstring,"%d",curValue);
        value = string(tempstring);
        setValue(section,key, value,usrcomment);
        save();
        ret = RET_MODIFY;
    }
